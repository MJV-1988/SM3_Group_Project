...
# cor.reduced <- round(cor(dat.num[,6:17]),2)
# kable(cor.reduced) %>%
#   kable_styling("striped", full_width = F) %>%
#   row_spec(0, angle = -90)
```

`r kable(cor.reduced, digits = 2, caption = "Converted Variable Types", format = "latex", booktabs = T)`
...
# set.seed(10)
# 
# ctrl <- rfeControl(functions = cforest, method = "repeatedcv",
#                    repeats = 5,
#                    verbose = FALSE)
# 
# lmProfile <- rfe(x, y,
...
# predictors(results)
# plot(results_2, type = c("g","o"))
# 
# # Reduced model
# train.set.reduced <- train.set
# train.set.reduced %>% select(PAY_0, PAY_2, PAY_3, y)
# 
# mdl_svm_radial <- train(y~., data=train.set.reduced, method="svmRadial",
...
```{r echo=FALSE}
# reduced_train.set <- select(PAY_0, EDUCATION, MARRIAGE,PAY_AMT1)
# reduced_test.set <- 
# 
...
# 
# importance <- varImp(mdl_logistic, scale=FALSE)
# 
# # Train linear SVM
...
# importance
# importance_dt <- varImp(mdl_dt, scale=FALSE)
# 
# # Train logistic regression model with subset of variables
# mdl_logistic_reduced <- train(y~PAY_0+, data=train.set, method="glm", family=binomial(), trControl=fitControl, preProcess=c("center", "scale"))
# 
# importance
# importance_dt
```
